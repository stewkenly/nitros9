...
REQUIRED_NITROS_BASE="a35e56a4dcfaeb808a508cb3f7c2d7ac242f91ad"
REQUIRED_XROAR_BASE="23d6f425d805de419f0297a62cbe60b6450e92e2"

if ! git merge-base --is-ancestor "$REQUIRED_NITROS_BASE" HEAD; then
    echo "FAIL: candidate is not descended from accepted S2B-2 base $REQUIRED_NITROS_BASE" >&2
    exit 1
fi
...
WORK="$(mktemp -d /tmp/supercoco-s2b3-runtime.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT
PROBE="$WORK/scgrfbarprobe"
VHD="$WORK/63SDC-S2B3.VHD"
...
os9 ident "$PROBE" | grep -Fqi 'scgrfbarprobe' || {
    echo 'FAIL: runtime probe module identity is not scgrfbarprobe' >&2
    exit 1
}

# Preserve the accepted in-place OS9Boot invariant.
cp "$BASE_VHD" "$VHD"
...
os9 copy -r "$PROBE" "$VHD,$CMD_DIR/scgrfbarprobe"
os9 attr "$VHD,$CMD_DIR/scgrfbarprobe" -e -pe -r -pr
...
"$XROAR" -ui null -ao null -machine coco3 -machine-cpu 6309 -ram 16384 \
  ...
  -type $'DOS\r' -timeout "${S2B3_TIMEOUT:-150}" >"$LOG" 2>&1
RC=$?
set -e
printf 'XRoar exit status: %d\n' "$RC"

if [[ -s "$CRASH" ]]; then
    echo 'FAIL: D.Crash trap fired during S2B-3 R1K BAR proof' >&2
    cat "$CRASH" >&2
    tail -n 200 "$LOG" >&2 || true
    exit 1
fi

FAIL_MARKER='scgrfbarprobe: S2B-3 R1K BAR fill FAIL'
required=(
  'S2B3 R1K BASE CLS PASS'
  'S2B3 R1K BAR1 PASS'
  'S2B3 R1K BAR2 PASS'
  'S2B3 R1K MIRROR PASS'
  'SuperCoCo S2B-3 R1K BAR fill PASS'
)

if grep -Fq "$FAIL_MARKER" "$SCREEN" 2>/dev/null || grep -Fq "$FAIL_MARKER" "$CONSOLE" 2>/dev/null; then
    echo 'FAIL: native S2B-3 probe reported R1K BAR failure' >&2
    cat "$SCREEN" >&2 || true
    cat "$CONSOLE" >&2 || true
    tail -n 200 "$LOG" >&2 || true
    exit 1
...
printf 'PASS: SuperCoCo S2B-3 normal GrfDrv BAR -> R1K FILL mirrored-buffer runtime proof\n'
printf 'NitrOS-9: %s\n' "$(git rev-parse HEAD)"

...
OUTPUT_VHD="${OUTPUT_VHD:-$(dirname "$SUPERCOCO_EMULATOR_ROOT")/_transfer/63SDC-S2A4.VHD}"
...
WORK="$(mktemp -d /tmp/supercoco-s2a4-prep.XXXXXX)"
...
os9 copy "$MODULE" "$OUTPUT_VHD,$CMD_DIR/scavdemo"
os9 attr "$OUTPUT_VHD,$CMD_DIR/scavdemo" -e -pe -r -pr

echo "Prepared S2A-4 image: $OUTPUT_VHD"
echo "Boot it with SuperCoCo XRoar and run: scavdemo"

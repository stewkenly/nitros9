...
REQUIRED_NITROS_BASE="dbaadc16a9aad81f6b226f0c9aca8f4ea974b606"
REQUIRED_XROAR_BASE="23d6f425d805de419f0297a62cbe60b6450e92e2"

if ! git merge-base --is-ancestor "$REQUIRED_NITROS_BASE" HEAD; then
    echo "FAIL: candidate is not descended from accepted S1 base $REQUIRED_NITROS_BASE" >&2
    exit 1
fi
...
WORK="$(mktemp -d /tmp/supercoco-s2b1-runtime.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT
PROBE="$WORK/scgrfprobe"
VHD="$WORK/63SDC-S2B1.VHD"
...
os9 ident "$PROBE" | grep -Fqi 'scgrfprobe' || {
    echo 'FAIL: runtime probe module identity is not scgrfprobe' >&2
    exit 1
}

# Start from the known-good CoCoSDC image. Do not run os9 gen here: it can
# relocate or replace the proven boot/storage stack. Patch only CoWin inside
# the existing contiguous OS9Boot allocation, preserving DD.BT and allocation.
cp "$BASE_VHD" "$VHD"
...
os9 copy -r "$PROBE" "$VHD,$CMD_DIR/scgrfprobe"
os9 attr "$VHD,$CMD_DIR/scgrfprobe" -e -pe -r -pr
...
"$XROAR" -ui null -ao null -machine coco3 -machine-cpu 6309 -ram 16384 \
  ...
  -type $'DOS\r' -timeout "${S2B1_TIMEOUT:-120}" >"$LOG" 2>&1
RC=$?
set -e
printf 'XRoar exit status: %d\n' "$RC"

if [[ -s "$CRASH" ]]; then
    echo 'FAIL: D.Crash trap fired during S2B-1 lifecycle proof' >&2
    cat "$CRASH" >&2
    tail -n 200 "$LOG" >&2 || true
    exit 1
fi

PASS_MARKER='SuperCoCo S2B-1 GrfDrv lifecycle PASS'
FAIL_MARKER='scgrfprobe: S2B-1 lifecycle FAIL'

if grep -Fq "$FAIL_MARKER" "$SCREEN" 2>/dev/null || grep -Fq "$FAIL_MARKER" "$CONSOLE" 2>/dev/null; then
    echo 'FAIL: native S2B-1 probe reported lifecycle failure' >&2
    cat "$SCREEN" >&2 || true
    cat "$CONSOLE" >&2 || true
    tail -n 200 "$LOG" >&2 || true
    exit 1
...
if ! grep -Fq "$PASS_MARKER" "$SCREEN" 2>/dev/null && ! grep -Fq "$PASS_MARKER" "$CONSOLE" 2>/dev/null; then
    echo "FAIL: missing runtime marker: $PASS_MARKER" >&2
    echo '--- screen ---' >&2
    cat "$SCREEN" >&2 || true
    echo '--- console ---' >&2
    cat "$CONSOLE" >&2 || true
...
printf 'PASS: SuperCoCo S2B-1 production CoWin/GrfDrv lifecycle runtime proof\n'
printf 'NitrOS-9: %s\n' "$(git rev-parse HEAD)"
